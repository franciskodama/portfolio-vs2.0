...
const Reason = () => {
  ...
  const createGrid = () => {
    for (let i = 0; i < arrOfLettersBright.length; i++) {
      arrOfDiv.push(
        <div
          key={i}
          className='reason__reveal-bright border border-white/20 uppercase h-[1.5em] w-[1.5em] text-center font-main-light text-[1.6rem] bg-dark relative opacity-0 transition-all duration-2000 ease-in-out -translate-x-[1000px] md-custom:text-[2rem] xl-custom:h-[1.3em] xl-custom:w-[1.3em] xl-custom:text-[3.8rem]'
        >
    ...
    for (let l = 0; l < arrOfLettersDark.length; l++) {
      arrOfDiv.push(
        <div
          key={l + 100}
          className='reason__reveal-dark border border-white/20 uppercase h-[1.5em] w-[1.5em] text-center font-main-light text-[1.6rem] bg-dark text-third relative opacity-0 transition-all duration-2000 ease-in-out translate-x-[1000px] md-custom:text-[2rem] xl-custom:h-[1.3em] xl-custom:w-[1.3em] xl-custom:text-[3.8rem]'
        >
          {arrOfLettersDark[l]}
        </div>
      );
  ...
  return (
    <section className='section flex flex-col max-w-[94em] mx-auto' id='reason'>
      <div
        className='flex flex-wrap gap-[0.3em] justify-center mt-[15em] mb-[10em]'
        ref={gridRef}
      >
        {createGrid()}
      </div>
